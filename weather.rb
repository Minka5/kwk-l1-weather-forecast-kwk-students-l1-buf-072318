...
def weather_report(weather)
  "the weather in #{location(weather)} is #{description(weather)}.The temperature is #{temperature(weather)} degrees Farenheit."
end
...
def dont_know_names(weather)
  weather.each do |key,value|
    puts "the #{key} is  #{value}."
  end 
end

dont_know_names(weather)
